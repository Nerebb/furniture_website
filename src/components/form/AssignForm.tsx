...
const AssignForm = ({ type, btnProps, keepOpen }: TFormProps) => {
    ...
    return (
        <Modal
            btnProps={btnProps}
            keepOpen={keepOpen}
            onClose={() => setFormType(type)}
        >
            <Dialog.Panel className="w-full min-w-[340px] xl:w-[1200px] 2xl:w-[1600px] transform overflow-hidden rounded-3xl lg:rounded-none lg:rounded-br-4xl bg-white p-2 text-left align-middle shadow-xl transition-all flex dark:bg-priBlack-700" >
                {/* Login */}
                <Dialog.Title
                    as='div'
                    className={'grow my-auto p-4 md:p-8 lg:p-12 2xl:p-24'}
                >
                    {/* Title */}
                    <div className='text-3xl font-semibold space-y-5 mb-10 md:text-2xl dark:text-white'>
                        <h2 className='lg:mb-14 flex gap-2 items-center'><CubeIcon className='w-8 h-8' /><span>Nereb</span></h2>
                        {formType === 'register' && <h1>Create an account</h1>}
                        {browserWidth > 468 && <p className='text-xl font-normal md:whitespace-nowrap md:text-base'>Let{"\'"}s get started with your 30 day free trial.</p>}
                    </div>
                    ...
                    >
                        {({ handleReset, isSubmitting, dirty }) => (
                            <Form className={classNames(
                                'flex flex-col space-y-7 sm:space-y-10'
                            )}>
                                ...
                                <div className={classNames(
                                    ...
                                    {formType !== 'register' &&
                                        <h1 className='flex-center dark:text-white'>
                                            Don{'\''}t have account ?
                                            <span
                                                className='hover:font-semibold underline text-priBlue-500 cursor-pointer ml-1'
                                    ...
                                    <Button
                                        text={isSubmitting || submitError ? "" : initForm.btnText}
                                        type='submit'
                                        modifier='w-full flex justify-center py-3 sm:py-4 gap-2 text-xl dark:text-white'
                                        variant={submitError ? 'outline' : 'fill'}
                                    >
                                        {submitError && (
                                            <div className={classNames(
                                                'w-full capitalize text-base',
                                                {
                    ...
                    {type !== 'moreInfo' && (
                        <div className='mt-5 space-y-5'>
                            <Button text='Sign in with google' modifier='w-full flex justify-center py-3 sm:py-4 gap-2 text-xl dark:text-white dark:border-white' variant='outline'
                            ...
                            <Button text='Sign in with github' modifier='w-full flex justify-center py-3 sm:py-4 gap-2 text-xl dark:text-white dark:border-white' variant='outline'
                                onClick={() => handleSignIn('github')}
                            >
                                {/* GitHubSVG */}
                                <div className='flex justify-center items-center rounded-sm w-8 h-8 p-1 dark:text-white'>
                ...
                <Dialog.Description
                    as='div'
                    className={'relative w-3/5 aspect-square group hidden lg:block'}
                >
